...
function parseJsonValue(raw, fieldName, fallback = {}) {
...
export function loadImageHostConfig(overrides = {}) {
  const uploadUrl = overrides.uploadUrl || process.env.IMAGE_HOST_UPLOAD_URL;
  if (!uploadUrl) return null;
  return {
    uploadUrl,
    method: overrides.method || process.env.IMAGE_HOST_METHOD || 'POST',
    fileField: overrides.fileField || process.env.IMAGE_HOST_FILE_FIELD || 'file',
    headers:
      overrides.headers != null
        ? parseJsonValue(overrides.headers, 'runtime.imageHost.headers')
        : parseJsonEnv('IMAGE_HOST_HEADERS_JSON'),
    formFields:
      overrides.formFields != null
        ? parseJsonValue(overrides.formFields, 'runtime.imageHost.formFields')
        : parseJsonEnv('IMAGE_HOST_FORM_FIELDS_JSON'),
    responseUrlPath: overrides.responseUrlPath || process.env.IMAGE_HOST_RESPONSE_URL_PATH || '',
    urlPrefix: overrides.urlPrefix || process.env.IMAGE_HOST_URL_PREFIX || '',
    successPath: overrides.successPath || process.env.IMAGE_HOST_SUCCESS_PATH || '',
    successValue: overrides.successValue || process.env.IMAGE_HOST_SUCCESS_VALUE || '',
  };
}
